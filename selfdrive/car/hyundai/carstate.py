...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp2, cp_cam):
    ...
    self.scc11init = copy.copy(cp_cam.vl["SCC11"]) if Params().get('SccHarnessPresent') else copy.copy(cp.vl["SCC11"])
    self.scc12init = copy.copy(cp_cam.vl["SCC12"]) if Params().get('SccHarnessPresent') else copy.copy(cp.vl["SCC12"])
    self.fca11init = copy.copy(cp_cam.vl["FCA11"]) if Params().get('SccHarnessPresent') else copy.copy(cp.vl["FCA11"])
